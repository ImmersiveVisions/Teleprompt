...
const SCRIPTS_DIR = process.env.SCRIPTS_DIR || path.join(__dirname, 'public', 'scripts');
...
function processHtmlFile(sourceFile, destinationFile) {
  ...
  try {
    ...
    // Simple regex-based approach to add styles
    let processedContent = htmlContent;
    
    // Check if there's a style tag
    if (processedContent.includes('</style>')) {
      // Add our styles to the existing style tag
      processedContent = processedContent.replace('</style>', `${customStyles}\n</style>`);
    } else if (processedContent.includes('</head>')) {
      // Add a style tag if none exists
      processedContent = processedContent.replace('</head>', `<style type="text/css">${customStyles}</style>\n</head>`);
    } else {
      // If no head tag, add both
      processedContent = processedContent.replace('<html', `<html>\n<head><style type="text/css">${customStyles}</style></head>`);
    }
    
    // Add a body style attribute for background color
    if (processedContent.includes('<body')) {
      processedContent = processedContent.replace(/<body([^>]*)>/i, '<body$1 style="background-color: black !important; color: white !important;">');
    }
    
    // Write the processed file
    fs.writeFileSync(destinationFile, processedContent);
...
// Run the script
console.log('Script Converter Tool (Simple Version)');
console.log('=====================================');
