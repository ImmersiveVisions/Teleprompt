    ...
    case 'CONTROL':
      ...
      switch (message.action) {
        ...
        case 'JUMP_TO_POSITION':
          // Simple prominent log message
          console.log('');
          console.log('********************************************************************');
          console.log('********** WS SERVER RECEIVED SCROLL COMMAND **********');
          console.log('********************************************************************');
          console.log('');
          
          // Handle both simple number and complex object formats
          if (typeof message.value === 'object' && message.value !== null) {
            console.log('===== [WS SERVER] Received enhanced JUMP_TO_POSITION data:', message.value);
            // Store the entire object in sharedState
            sharedState.scrollData = message.value;
            // Also update the position for backward compatibility
            sharedState.currentPosition = message.value.position;
          } else {
            // Simple numeric value (old format)
            console.log('===== [WS SERVER] Received simple JUMP_TO_POSITION:', message.value);
            sharedState.currentPosition = message.value;
            // Clear any existing scrollData
            sharedState.scrollData = null;
          }
          // Log the current shared state that will be broadcast
          console.log('===== [WS SERVER] Updated shared state:', JSON.stringify({
            ...sharedState,
...
const broadcastState = () => {
  ...
  // Log when broadcasting state that contains position info
  if (sharedState.currentPosition > 0) {
    // Simple prominent log message
    console.log('');
    console.log('********************************************************************');
    console.log('********** WS SERVER BROADCASTING SCROLL POSITION: ' + sharedState.currentPosition + ' **********');
    console.log('********************************************************************');
    console.log('');
    
    console.log(`===== [WS SERVER] Broadcasting state with position: ${sharedState.currentPosition} to ${connections.length} clients`);
  }
  ...
  if (sharedState.currentPosition > 0) {
    console.log(`===== [WS SERVER] Sent to ${sentCount} active clients`);
  }
};
